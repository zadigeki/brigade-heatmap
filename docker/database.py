...
class DatabaseManager:
    ...
    def upsert_device(self, device_data: Dict[str, Any]) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute("SELECT id FROM devices WHERE terid = ?", (device_data.get('terid'),))
                ...
                if existing:
                    ...
                    """
                    values = (
                        device_data.get('carlicense'),
                        device_data.get('sim'),
                        device_data.get('channel'),
                        device_data.get('platecolor'),
                        device_data.get('groupid'),
                        device_data.get('cname'),
                        device_data.get('devicetype'),
                        device_data.get('linktype'),
                        device_data.get('deviceusername'),
                        device_data.get('devicepassword'),
                        device_data.get('registerip'),
                        device_data.get('registerport'),
                        device_data.get('transmitip'),
                        device_data.get('transmitport'),
                        device_data.get('en'),
                        device_data.get('companybranch'),
                        device_data.get('companyname'),
                        device_data.get('terid')
                    )
        ...
        for device in devices:
            try:
                with self._get_connection() as conn:
                    ...
                    # Check if device exists
                    cursor.execute("SELECT id FROM devices WHERE terid = ?", (device.get('terid'),))
                    existing = cursor.fetchone()
                    ...
                    if existing:
                        ...
                        """
                        values = (
                            device.get('carlicense'),
                            device.get('sim'),
                            device.get('channel'),
                            device.get('platecolor'),
                            device.get('groupid'),
                            device.get('cname'),
                            device.get('devicetype'),
                            device.get('linktype'),
                            device.get('deviceusername'),
                            device.get('devicepassword'),
                            device.get('registerip'),
                            device.get('registerport'),
                            device.get('transmitip'),
                            device.get('transmitport'),
                            device.get('en'),
                            device.get('companybranch'),
    ...
    def insert_alarm(self, alarm_data: Dict[str, Any]) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(insert_sql, values)
                conn.commit()
                logger.debug(f"Inserted alarm for device {alarm_data.get('terid')}")
                return True
    ...
    def insert_alarms_batch(self, alarms: List[Dict[str, Any]]) -> Dict[str, int]:
        """
        Batch insert multiple alarm records
        Returns dict with counts of inserted/failed records
        """
